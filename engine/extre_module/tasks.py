...
from engine.extre_module.custom_nn.stem.mystem import  MyStem
from engine.extre_module.custom_nn.featurefusion.emfm import EMFM
from engine.extre_module.custom_nn.neck.FDPN_EMFM import EFocusFeature
    ...
    def __init__(self, \
        ...
        if pretrained:  
            ...
            try:
                ...
                # need_keep_key_prefix_list = ['1.*', '2.*', '3.*', '4.*']
                # need_pop_key_list = []
...
def parse_module(d, i, f, m, args, ch, nc=None, eval_spatial_size=None):  
    ...
    c2 = ch[-1]
    if m in {StemBlock, HG_Stage, MyStem}:
        ...
        c2 = ch[f[0]]  
    elif m in {MFM, MultiScalePCA, MultiScalePCA_Down, EMFM}: # featurefusion
        c1 = [ch[i] for i in f]    
        c2 = args[0]
        args = [c1, c2, *args[1:]]     
    elif m in {FocusFeature, EFocusFeature}: 
        c1 = [ch[i] for i in f]     
        c2 = c1[1]    
        args = [c1, *args]
